			...
			case 166:
				...
				if(errn == 0) {
					if(eventCode == 0) {
						...
						if (errn == 0) {
							errn =  (exiFrag->RootCertificateID.charactersLen - 2) <= EXIFragment_RootCertificateID_CHARACTERS_SIZE ? 0 : EXI_ERROR_STRINGVALUES_OUT_OF_BOUND;
						}
						if (errn == 0) {
							if (exiFrag->RootCertificateID.charactersLen >= 2) {
								...
								errn = decodeCharacters(stream, exiFrag->RootCertificateID.charactersLen, exiFrag->RootCertificateID.characters);
							} else {
								/* string table hit */
								errn = EXI_ERROR_STRINGVALUES_NOT_SUPPORTED;
							}
